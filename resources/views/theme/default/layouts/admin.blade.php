	...
	<nav class="navbar navbar-inverse navbar-fixed-top">
      <div class="container">
        <div class="navbar-header">
          ...
          <a class="navbar-brand" href="{{-- site_url --}}">{{ get_settings('company_name') }}</a>
        </div>
        <div id="navbar" class="navbar-collapse collapse">
            <ul class="nav navbar-nav navbar-right"> 
            <?php if (is_user_logged_in()) { ?>  
                <li><a href="#">Welcome {{ get_loggedin_info('username')}}</a></li>
                <li><a href="{{-- ('authentication/logout') --}}">{{ __('messages.logout') }}</a></li>
            <?php } else { ?>           
                <li><a href="{{--('authentication/index') --}}">Login</a></li>
                <li><a href="{{--('authentication/signup') --}}">Sign Up</a></li>
            <?php } ?>    
            </ul>
        </div><!--/.navbar-collapse -->
      </div>
